...
module.exports = {
  ...
  updateNameEmail: function (sqlCon, data, callback) {
    sqlCon.query(
      `UPDATE USER set name =${sqlCon.escape(
        data.name
      )}, surname=${sqlCon.escape(data.surname)} WHERE userId = ${sqlCon.escape(
        data.userId
      )}`,
      callback
    );
  ...
  updatePasswd: function (sqlCon, data, callback) {
    sqlCon.query(
      `UPDATE USER set passwd =${sqlCon.escape(
        data.passwd
      )} WHERE userId = ${sqlCon.escape(data.userId)}`,
      callback
    );
  },
  User: User,
